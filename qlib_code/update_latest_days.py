...
class QlibDataConverter:
    ...
    def get_latest_trading_date(self):
        ...
        try:
            query = text("""
                ...
                FROM data_prepared_new.data_stock 
    ...
    def get_hfq_data_from_sql(self, code, target_date):
        ...
        try:
            ...
            # 查询股票数据
            query = text("""
                SELECT valuation_date, code, open, high, low, close, volume, amt, adjfactor 
                FROM data_stock 
                WHERE code = :code 
    ...
    def format_for_qlib(self, df, code=None):
        ...
        # Ensure factor column exists and has no NaN (qlib expects a valid factor field)
...
def main():
    ...
    if env_date:
        try:
            if '-' in env_date:
                target_date = datetime.strptime(env_date, '%Y-%m-%d').strftime('%Y%m%d')
            elif len(env_date) == 8 and env_date.isdigit():
                target_date = env_date
            else:
                # unknown format, pass through and let internal funcs try to handle
                target_date = env_date
        except Exception:
            target_date = env_date
        
        try:
            converter.csv_output_dir = os.path.join(output_dir, target_date)
            os.makedirs(converter.csv_output_dir, exist_ok=True)
        except Exception:
            pass
    else:
        target_date = None
    print(target_date)
    converter.process_all_stocks(market=market, batch_size=batch_size, target_date=target_date)
    converter.process_all_indices(market=market, target_date=target_date)
