    ...
    def generate(self, **kwargs):
        ...
        try:
            if getattr(self, "model", None) is not None:
                # Save under the same artifact path as other record artifacts
                self.recorder.save_objects(artifact_path=self.get_path(), trained_model=self.model)
                logger.info(
                    f"Signal record 'trained_model' has been saved as the artifact of the Experiment {self.recorder.experiment_id}"
                )
        except Exception as _e:
            logger.warning(f"Failed to save trained model artifact: {_e}")
        # print out results
